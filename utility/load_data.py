...
class Data(object):
    ...
    def sample_valid(self):
        if self.batch_size <= self.n_users:
            users = rd.sample(self.valid_set.keys(), self.batch_size)
        else:
            users = [rd.choice(self.exist_users) for _ in range(self.batch_size)]

        def sample_pos_items_for_u(u, num):
            pos_items = self.valid_set[u]
            n_pos_items = len(pos_items)
            pos_batch = []
            while True:
                if len(pos_batch) == num: break
        ...
        def sample_neg_items_for_u(u, num):
            ...
            while True:
                if len(neg_items) == num: break
                neg_id = np.random.randint(low=0, high=self.n_items, size=1)[0]
                if neg_id not in (self.valid_set[u] + self.train_items[u]) and neg_id not in neg_items:
                    neg_items.append(neg_id)
            return neg_items
    ...
    def print_statistics(self):
        ...
        print('n_interactions=%d' % (self.n_train + self.n_valid + self.n_test))
        print('n_train=%d, n_valid=%d, n_test=%d, sparsity=%.5f' % (self.n_train, self.n_valid, self.n_test, (self.n_train + self.n_test)/(self.n_users * self.n_items)))
