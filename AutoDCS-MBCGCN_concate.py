...
def load_logging():
    ...
    log_filename = args.weights_path + f'buy/AutoDCS-MBCGCN_no_w_{t.strftime("%Y%m%d_%H%M%S")}.log'
...
class ADCS_jdata(object):
    ...
    def _create_mbcgcn_embed(self, user_embedding, item_embedding):
        ...
        u_embeddings1 = tf.concat([u_g_embeddings, tf.to_float(u_label_embeddings1)], axis=1)
        i_embeddings1 = tf.concat([i_g_embeddings, tf.to_float(i_label_embeddings1)], axis=1)
    ...
    def _create_mbcgcn_embed2(self, user_embedding, item_embedding):
        ...
        u_embeddings2 = tf.concat([u_g_embeddings2, tf.to_float(u_label_embeddings2)], axis=1)
        i_embeddings2 = tf.concat([i_g_embeddings2, tf.to_float(i_label_embeddings2)], axis=1)
    ...
    def _create_mbcgcn_embed4(self, user_embedding, item_embedding):
        ...
        u_embeddings4 = tf.concat([u_g_embeddings4, tf.to_float(u_label_embeddings4)], axis=1)
        i_embeddings4 = tf.concat([i_g_embeddings4, tf.to_float(i_label_embeddings4)], axis=1)
    ...
    def _create_mbcgcn_embed3(self):
        ...
        u_embeddings3 = tf.concat([u_g_embeddings3, tf.to_float(u_label_embeddings3)], axis=1)
        i_embeddings3 = tf.concat([i_g_embeddings3, tf.to_float(i_label_embeddings3)], axis=1)
